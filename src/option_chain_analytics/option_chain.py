...
class SlicesChain:
    ...
    def get_atm_call_id(self,
        ...
        if slice_id is None and mat_date is None:
            raise ValueError("provide slice_id or mat_date")
        if slice_id is None:
...
def find_idx_nearest_element(value: float,
    ...
    """
    Select an element from a numeric grid.

    ``BELOW`` and ``ABOVE`` are strict when the grid has a value on the
    requested side. At the grid boundaries the nearest endpoint is returned.
    """
    ...
    if nearest_strike_on_grid == NearestStrikeOnGrid.BELOW:
        idx = np.searchsorted(sorted_array, value, side="left") - 1
        return idx_sorted[max(idx, 0)]

    if nearest_strike_on_grid == NearestStrikeOnGrid.ABOVE:
        idx = np.searchsorted(sorted_array, value, side="right")
        return idx_sorted[min(idx, len(a) - 1)]
